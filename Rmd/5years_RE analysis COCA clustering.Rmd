...
phenoclean <- phenoclean %>%
  ...
  mutate(os_time = timelastfu) %>% 
  mutate(os_time_5year = case_when(
    os_time < day(seconds_to_period(dyears(5)))              ~ os_time,
    TRUE                                                     ~ day(seconds_to_period(dyears(5)))
  )) %>% 
  mutate(os_event_5year = case_when(
    os_time < day(seconds_to_period(dyears(5)))              ~ os_event,
    TRUE                                                     ~ 0
  ))
...
write_rds(met_data, "met_data.rds")
```
