...
tbl1 <- met_data %>%
  ...
  tbl_uvregression(method = survival::coxph,
                   y = (Surv(time = met_data$os_time_months,
                             event = met_data$os_event)),
                   exponentiate = TRUE) %>%
  bold_p(t = .05) %>% 
  add_nevent(location = "level") %>% add_n(location = "level") %>%
  bold_labels() %>% italicize_levels()
tbl2 <- coxph(Surv(time = met_data$os_time_months, 
...
PH assumption
```{r}
coxph(Surv(time = met_data$os_time_months,
                   event = met_data$os_event) ~ coca_RE_cluster + refage + stage_cat,
      data = met_data)  %>%
  cox.zph()
```
...
ggsurvplot(survfit(Surv(os_time_months, os_event) ~ coca_RE_cluster, 
                   data=met_data),
           title = "OS Analysis",
...
<!-- coxph(Surv(time = adj_data$os_time_months, -->
...
coxph(Surv(time = distant_data$os_time_months,
...
coxph(Surv(time = serous_data$os_time_months,
...
<!-- coxph(Surv(time = black_data$os_time_months, -->
