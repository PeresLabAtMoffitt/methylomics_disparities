...
<span style="color: white;">LTR Retrotransposons analysis in AACES and NCOCS data</span>
...
# write_rds(remp_res_ERV, "remp_res_ERV_aggregate.rds")
remp_res_ERV <- read_rds(paste0(here::here(), "/remp_res_ERV_aggregate.rds"))
details(remp_res_ERV)
...
# write_rds(remp_res_ERV, "remp_res_ERV_annotation.rds")
remp_res_ERV <- read_rds(paste0(here::here(), "/remp_res_ERV_annotation.rds"))
rempAnnot(remp_res_ERV)
...
# Need to bind ERV with chr and symol
# Data results with the B value and RE.Index
ERV_beta_results <-
  annot_ERV_beta_results %>%
  # Merge with annotation, get #chr, start, end
  left_join(., as_tibble(remp_res_ERV@metadata$REannotation), by = c("Index" = "Index"))
...
for (i in chr_vec){
  ...
  data_chr <- ERV_beta_results %>%
  filter(seqnames == i) %>%
  # Merge with refgene, get gene symbol
  left_join(., as_tibble(remp_res_ERV@metadata$refGene), by = c("seqnames", "strand"), suffix = c("_RE", "_gene")) %>%
...
ERV_beta_results <- data
rm(chr_vec, i, data, data_chr)
```
...
row_col = rowAnnotation(Chromosome = c(ERV_beta_results$seqnames[1:200]),
                        Gene = c(ERV_beta_results$GeneSymbol[1:200]),
                        col = list(
                          na_col = "black")
)
Heatmap(df_map1, name = " ",
