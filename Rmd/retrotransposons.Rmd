...
<span style="color: red;">From what I uderstood, I don't need to furnish the CpG location info. Correct?</span>  
...
groomed_data <- read_rds("groomed_data.rds")
groomed_data@NAMES
...
# write_rds(remp_res_Alu, "remp_res_Alu_aggregate.rds")
remp_res_Alu <- read_rds("remp_res_Alu_aggregate.rds")
details(remp_res_Alu)
...
# write_rds(remp_res_Alu, "remp_res_Alu_annotation.rds")
remp_res_Alu <- read_rds("remp_res_Alu_annotation.rds")
rempAnnot(remp_res_Alu)
...
```{r}
# Need to bind Alu with chr and symol
# Data results with the B value and RE.Index
Alu_beta_results <- 
  annot_Alu_beta_results %>% 
  # Merge with annotation, get #chr, start, end
  left_join(., as_tibble(remp_res_Alu@metadata$REannotation), by = c("Index" = "Index")) 
...
for (i in chr_vec){
  ...
  data_chr <- Alu_beta_results %>% 
  filter(seqnames == i) %>% 
  # Merge with refgene, get gene symbol
  left_join(., as_tibble(remp_res_Alu@metadata$refGene), by = c("seqnames", "strand"), suffix = c("_RE", "_gene")) %>% 
...
row_col = rowAnnotation(Chromosome = c(Alu_beta_results$seqnames[1:200]),
                        Gene = c(Alu_beta_results$GeneSymbol[1:200]),
                        col = list(
                          na_col = "black")
)
Heatmap(df_map1, name = " ",
